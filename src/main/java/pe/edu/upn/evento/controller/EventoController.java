...
public class EventoController {
	...
	public boolean SoloLetras(String cadena ) {
		for (int i = 0; i < cadena.length(); i++)
		{
			...
			if  ((valorASCII < 65 || valorASCII > 90) && valorASCII!=164 && valorASCII!=165 )
				return false; //Se ha encontrado un caracter que no es letra 164ñ 165Ñ
		}
		return true;
	...
	public String escoger(@PathVariable Integer id, SessionStatus session, Model model) {
		...
		try {
			...
			if (optional.isPresent()) {
				List<Evento> listaUsuario =  eventoService.findAll();
				model.addAttribute("eventosAll",listaUsuario);
